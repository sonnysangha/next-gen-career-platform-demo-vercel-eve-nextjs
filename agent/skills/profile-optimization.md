---
description: Use when the user wants to rewrite or improve their profile — headline, about section, or experience bullets — for a target role.
---

# Profile optimization

Playbook for producing a strong profile rewrite with `create_profile_rewrite`.
Always load the user's real data with `get_user_profile` first, and, when a target
role is in play, `analyze_skill_gap` so the rewrite leans into real strengths.
...
- Quantify wherever the data exists (%, $, time saved, scale, users). If there's no
  number, ask via `ask_question` rather than inventing one.
...
- In the `explanation` field, say what changed and why in 2–4 tight bullets so the
  old-vs-new diff is easy to scan.

## Guardrails

- Never fabricate titles, dates, metrics, or skills. Ground everything in real data.
- `create_profile_rewrite` does NOT save. Show it, then call `save_profile_draft`
