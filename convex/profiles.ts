...
/**
 * Public profile page payload: user + profile + experiences + skills + recent
 * posts. Returns null when no user has that username.
 */
export const getProfileByUsername = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Recent posts by this author. posts has no author index, so scan newest
    // first and keep this author's up to a small cap.
    const recentPosts = [];
    for await (const post of ctx.db
      ...
      if (post.authorId === user._id) {
        ...
        if (recentPosts.length >= 10) break;
      }
    }

    return { user, profile, experiences, skills, recentPosts };
  },
});
